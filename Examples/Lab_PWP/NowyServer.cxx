...
int ReceivePoint(igtl::Socket * socket, igtl::MessageHeader * header)
{
  ...
  igtl::PointMessage::Pointer send;
  send = igtl::PointMessage::New();
  send->SetDeviceName("PointSender");
  ...
  if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
    ...
    for (int i = 0; i < nElements; i ++)
	  ...
	  negpoint->SetPosition(p);
	  negpoint->SetRadius(pointElement->GetRadius());
	  negpoint->SetOwner(pointElement->GetOwner());


	  send->AddPointElement(negpoint);
	}


	igtl::Sleep(4000);
	send->Pack();
	socket->Send(send->GetPackPointer(), send->GetPackSize());
  }
